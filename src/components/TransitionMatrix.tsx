                ...
                >
                  <div className="flex items-center justify-between mb-1.5">
                    ...
                    <span className={`text-xs font-bold font-mono px-2 py-0.5 rounded ${
                      isToBuilt ? 'bg-rose-500/20 text-rose-300' : 'bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300'
                    }`}>
                      {t.percentage_of_aoi}% of AOI
                    </span>
                  </div>

                  <div className="flex items-center gap-2 font-bold text-slate-900 dark:text-white text-sm my-1">
                    <span className="truncate">{t.from_label}</span>
                    <ArrowRight className="w-4 h-4 text-sky-400 shrink-0" />
                    <span className={`truncate ${isToBuilt ? 'text-rose-400' : 'text-slate-800 dark:text-slate-200'}`}>
                      {t.to_label}
                    </span>
                  </div>

                  <div className="flex items-center justify-between text-xs text-slate-500 dark:text-slate-400 mt-2 pt-2 border-t border-slate-200/80 dark:border-slate-800/80">
                    <span>Observed Shift</span>
                    <span className="font-extrabold text-slate-900 dark:text-white text-sm font-mono">{t.hectares} ha</span>
                  </div>
                </div>
              );
            })}
          ...
          <table className="w-full text-xs text-left border-collapse">
            <thead>
              <tr className="border-b border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950">
                <th className="p-2.5 text-slate-500 dark:text-slate-400 font-semibold uppercase tracking-wider text-[11px]">
                  Past ({pastYear}) ↓
                </th>
                {matrixClasses.map((cls) => (
                  <th key={cls} className="p-2.5 text-slate-700 dark:text-slate-300 font-semibold text-center text-[11px]">
                    {labels[cls]}
                  </th>
                ))}
              </tr>
            ...
            <tbody>
              {matrixClasses.map((fromCls) => (
                <tr key={fromCls} className="border-b border-slate-800/60 hover:bg-slate-800/30 transition-colors">
                  <td className="p-2.5 font-bold text-slate-900 dark:text-white bg-slate-950/60">
                    {labels[fromCls]}
                  </td>
                  {matrixClasses.map((toCls) => {
                    ...
                    if (isDiagonal && val > 0) {
                      bgStyle = 'bg-slate-800/40 text-slate-800 dark:text-slate-200 font-semibold';
                    } else if (isConversionToBuilt) {
                      bgStyle = 'bg-rose-500/20 text-rose-300 font-bold border border-rose-500/30';
                    } else if (val > 0) {
                      bgStyle = 'text-slate-700 dark:text-slate-300 font-medium';
                    }
      ...
      {activeTab === 'composition' && (
        <div className="space-y-3">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            {composition.map((c) => (
              <div key={c.type} className="p-3 bg-slate-50 dark:bg-slate-950 rounded-xl border border-slate-200 dark:border-slate-800">
                <div className="flex items-center justify-between mb-2">
                  ...
                  <span className={`text-xs font-mono font-bold px-2 py-0.5 rounded ${
                    c.delta_ha > 0 ? 'bg-rose-500/20 text-rose-400' : c.delta_ha < 0 ? 'bg-amber-500/20 text-amber-400' : 'bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400'
                  }`}>
                ...
                <div className="grid grid-cols-2 gap-2 text-xs">
                  <div className="bg-white dark:bg-slate-900 p-2 rounded border border-slate-200 dark:border-slate-800">
                    <div className="text-[10px] text-slate-500 dark:text-slate-400 uppercase">Past ({pastYear})</div>
                    <div className="font-bold text-slate-800 dark:text-slate-200 mt-0.5">{c.past_ha} ha ({c.past_pct}%)</div>
                  </div>
                  <div className="bg-white dark:bg-slate-900 p-2 rounded border border-slate-200 dark:border-slate-800">
                    <div className="text-[10px] text-slate-500 dark:text-slate-400 uppercase">Present ({presentYear})</div>
